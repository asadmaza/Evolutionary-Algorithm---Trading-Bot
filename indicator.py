...
def get_indicators(candles: pd.DataFrame, params: list[dict]) -> list[pd.Series]:
  ...
  return [INDICATORS[i].ind_fn(*[candles[c] for c in INDICATORS[i].columns], **params[i]) for i in range(NUM_INDICATORS)]
...
def mutate_params(params: list[dict], prob: float = 1.0) -> list[dict]:
  ...
  for i in range(NUM_INDICATORS):
    if random.random() < prob: params[i] = INDICATORS[i].mutator_fn(params[i])
