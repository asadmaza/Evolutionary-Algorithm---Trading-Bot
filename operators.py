...
def __mutate_numeric_indicator_params(
  ...
  mutation_mask = np.random.random(
      size=len(params)) < ELEMENT_WISE_MUTATION_PROB
...
def __mutate_constants(chromosome: Chromosome):
  ...
  mutation_mask = (
      np.random.random(
          size=len(
              chromosome.constants)) < ELEMENT_WISE_MUTATION_PROB)
  changes = np.array(
      [
...
def __match_symbol_lengths(
  ...
  n_A1, n_B1, n_C1 = (count_symbols(parent1, point1, s)
                      for s in [parent1.A, parent1.B, parent1.C])
  n_A2, n_B2, n_C2 = (count_symbols(parent2, point2, s)
                      for s in [parent2.A, parent2.B, parent2.C])

  child.indicators = parent1.indicators[:n_A1] + parent2.indicators[n_A2:]
  child.candle_params = parent1.candle_params[:n_A1] + \
      parent2.candle_params[n_A2:]
  child.int_params = parent1.int_params[:n_A1] + parent2.int_params[n_A2:]
  child.float_params = parent1.float_params[:n_A1] + \
      parent2.float_params[n_A2:]
  child.candle_names = parent1.candle_names[:n_B1] + \
      parent2.candle_names[n_B2:]
  child.constants = np.append(
      parent1.constants[:n_C1], parent2.constants[n_C2:], 0)
