...
class Strategy():
  ...
  def buy_sum(self, t: int) -> float:
    ...
    return sum([self.buy_weights[i] * self.indicators[i][t]
  ...
  def sell_sum(self, t: int) -> float:
    ...
    return sum([self.sell_weights[i] * self.indicators[i][t]
               for i in range(indicator.NUM_INDICATORS)])
