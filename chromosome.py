...
class Chromosome:
  ...
  def to_str(self) -> str:
    ...
    dnf = " ∧ ".join(__disjunction_to_str(disj)
                     for disj in self.expression_list)
  ...
  def to_str_non_symbolic(self) -> str:
    ...
    def replacement_fn(
        match): return self.__replace_value_symbols(match, counters)
...
class ChromosomeHandler:
  ...
  def __init__(
    ...
    for module in modules:
      # Ignore 'adx' cause of "invalid value encountered in scalar divide"
      # warning
  ...
  def __gen_constant(self, chromosome: dict) -> float:
    """Add constant to chromosome"""
    chromosome.constants += [round(random.uniform(0,
                                   CONST_MAX), DECIMAL_PLACES)]
    return chromosome.C
  ...
  # ==================== MISC ====================
  def __attach_buy_sell_prefix(
          self,
          c: Chromosome,
          is_buy: bool) -> Chromosome:
